...
def github_topics_scraper(detailed=False, records=True):
    ...
    if type(detailed) != bool:
        raise Exception(
            "Expected boolean input for argument 'detailed' but got{}".format(
                type(detailed)
            )
        )
    if records < 0:
    ...
    if type(records) != int:
        if type(records) == bool:
            ...
            raise Exception(
                "Expected integer or boolean input for the argument 'records' but got{}".format(
                    type(records)
                )
            )

    print(
        "Scrapping GitHub topics{}...".format(" in detail" if detailed == True else "")
    )
    ...
    if records > len(Topics_details["Topic_URL"]):
        print(
            "There are only {} topics present on GitHub Topics webpage.".format(
                len(Topics_details["Topic_URL"])
            )
        )
...
def get_topic_details(records):
    ...
    # 'end' calculates how many pages to load based on the required no of records
        ...
        math.ceil(records / 30)
        if type(records) == int
        else (1 if records == True else 6)
    )
    while start_page <= end:
        url = "https://github.com/topics?page={}".format(
            start_page
    ...
    topics = []
    topic_ptags = soup_doc.find_all(
        "p", {"class": "f3 lh-condensed mb-0 mt-1 Link--primary"}, limit=records
    )
    ...
    topic_descs = []
    descr_ptags = soup_doc.find_all(
        "p", {"class": "f5 color-fg-muted mb-0 mt-1"}, limit=records
    )
    ...
    topic_urls = []
    topic_url_tags = soup_doc.find_all(
        "a", {"class": "no-underline flex-1 d-flex flex-column"}, limit=records
    )
...
def get_popular_repo_details(topic_urls):
    ...
    while i < len(
        topic_urls
    ):  # topic_urls -> scraped already, utilizing to scrape remaining data
        url = topic_urls[i] + "?o=desc&s=stars"  # creating url based on the topic
        r = requests.get(url)
        if r.status_code != 200:
            i -= 1
        else:
            pr_soup1 = BeautifulSoup(
                r.text, "html.parser"
            )  # creating beautiful soup object

            # popular repo name, usernam and URL, loacted in a-tag of first h3-tag
            h3_tags = pr_soup1.find_all(
                "h3", {"class": "f3 color-fg-muted text-normal lh-condensed"}, limit=1
            )
            ...
            if r.status_code != 200:
                ...
                star_span_tag = pr_soup2.find_all(
                    "span", {"id": "repo-stars-counter-star"}
                )
                stars = int(star_span_tag[0]["aria-label"].split()[0])

                # locating & extracting tags for forks counts
                forks_span_tag = pr_soup2.find_all(
                    "span", {"id": "repo-network-counter"}
                )
                forks = int(forks_span_tag[0]["title"].replace(",", ""))

                # locating & extracting tags for commits
                commit_span_tags = pr_soup2.find_all(
                    "span", {"class": "d-none d-sm-inline"}
                )
                commits = (
                    int(commit_span_tags[1].strong.text.replace(",", ""))
                    if len(commit_span_tags) == 2
                    else int(commit_span_tags[0].strong.text.replace(",", ""))
                )

                # locating & extracting tags for last committed time
                last_commit_atag = pr_soup2.find_all(
                    "a", {"class": "Link--secondary ml-2"}
                )
                last_updated = (
                    last_commit_atag[0].find_all("relative-time")[0]["datetime"]
                    if len(last_commit_atag) >= 1
                    else None
                )
